/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   cones.c                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: xriera-c <xriera-c@student.hive.fi>        +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/19 10:22:43 by xriera-c          #+#    #+#             */
/*   Updated: 2024/09/23 17:23:09 by xriera-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../inc/minirt.h"

static t_inters	*get_cone_inters(t_object *cone, t_cylvals *v)
{
	t_inters	*xs;

	xs = create_intersections(3);
	if (!xs)
		return (NULL);
	xs->t[0] = v->t0;
	xs->t[1] = v->t1;
	xs->objects[0] = cone;
	xs->objects[1] = cone;
	xs->count = 2;
	xs->objects[2] = NULL;
	return (xs);
}

t_inters	*cone_intersect(t_object *cone, float **ray)
{
	t_cylvals	v;

	v.a = pow(ray[1][0], 2) - pow(ray[1][1], 2) + pow(ray[1][2], 2);
	v.b = 2 * ray[0][0] * ray[1][0] - 2 * ray[0][1] * ray[1][1]
		+ 2 * ray[0][2] * ray[1][2];
	v.c = pow(ray[0][0], 2) - pow(ray[0][1], 2) + pow(ray[0][2], 2);
	if (equal_float(v.a, 0) == 1)
	{
		if (fabs(v.b) > EPS)
		{
			v.t0 = -v.c / (2 * v.b);
			v.t1 = v.t0;
		}
		return (get_cone_inters(cone, &v));
	}
	v.disc = pow(v.b, 2) - (4 * v.a * v.c);
	if (fabs(v.disc) < EPS)
		v.disc = 0;
	if (v.disc < 0)
		return (NULL);
	v.t0 = ((-v.b) - sqrt(v.disc)) / (2 * v.a);
	v.t1 = ((-v.b) + sqrt(v.disc)) / (2 * v.a);
	if (v.t0 > v.t1)
			ft_swap(&v.t0, &v.t1);
	return (get_cone_inters(cone, &v));
}
